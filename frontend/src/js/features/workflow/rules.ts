...
export function buildDeveloperConfigWithDefaults({
  ...
  return {
    ...
    ocrProfiles: Array.isArray(source.ocrProfiles) ? source.ocrProfiles : [],
    ocrProfileId: `${source.ocrProfileId || source.ocr_profile_id || ""}`.trim(),
    ocrBaseUrl: `${source.ocrBaseUrl || source.ocr_base_url || "https://api.mistral.ai"}`.trim(),
    ocrModel: `${source.ocrModel || source.ocr_model || "mistral-ocr-latest"}`.trim(),
