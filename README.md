...
- 浏览器端支持选择目标语言，并可在高级设置中同时控制并发数、QPS 与 RPM 限流
...
- 可选择目标语言（例如简中、繁中、English、日本語等）
- 可在高级控制里同时调整并发数、QPS、RPM
- 默认注入值仍然可由 `docker/web.env` 提供，但最终用户也可以在浏览器中覆盖

### Windows 下一键打开

当前仓库根目录提供了几个可直接双击的脚本：

- `启动 RetainPDF.cmd`
- `停止 RetainPDF.cmd`
- `重启 RetainPDF.cmd`

它们会直接调用 Docker CLI，不需要你手动打开 Docker Desktop 界面。
